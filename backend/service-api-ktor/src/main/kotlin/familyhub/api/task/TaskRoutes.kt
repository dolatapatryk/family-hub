...
fun Route.taskRoutes(tasks: TaskService, users: UserService) {
    route("/api/tasks") {
        get {
            ...
            val result = withContext(Dispatchers.IO) { tasks.list(filter) }
        ...
        get("/{id}") {
            val user = call.identifiedUser(users)
            val id = call.taskId()
            val task = withContext(Dispatchers.IO) { tasks.get(user, id) }
        ...
        post {
            ...
            val task = withContext(Dispatchers.IO) { tasks.create(user, command) }
            call.response.headers.append(HttpHeaders.Location, "/api/tasks/${task.id}")
        ...
        patch("/{id}") {
            ...
            val task = withContext(Dispatchers.IO) { tasks.update(user, id, command) }
            call.respond(task.toResponse())
        }
        ...
        post("/{id}/assign") {
            ...
            val task = withContext(Dispatchers.IO) { tasks.assignTo(user, id, assigneeId) }
            call.respond(task.toResponse())
        }
        ...
        post("/{id}/unassign") {
            val user = call.identifiedUser(users)
            val id = call.taskId()
            val task = withContext(Dispatchers.IO) { tasks.unassign(user, id) }
            call.respond(task.toResponse())
        }
        ...
        post("/{id}/complete") {
            val user = call.identifiedUser(users)
            val id = call.taskId()
            val task = withContext(Dispatchers.IO) { tasks.complete(user, id) }
            call.respond(task.toResponse())
        }
        ...
        post("/{id}/reopen") {
            val user = call.identifiedUser(users)
            val id = call.taskId()
            val task = withContext(Dispatchers.IO) { tasks.reopen(user, id) }
            call.respond(task.toResponse())
        }
        ...
        post("/{id}/archive") {
            val user = call.identifiedUser(users)
            val id = call.taskId()
            val task = withContext(Dispatchers.IO) { tasks.archive(user, id) }
            call.respond(task.toResponse())
        }
    }
}
